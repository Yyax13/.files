#!/usr/bin/env bash
PORT=$(cat "/home/yyax/dotfiles/hypr/.config/hypr/.dual-graph/run/claude/mcp_port" 2>/dev/null || echo 8080)
...
if [[ -f "/home/yyax/dotfiles/hypr/.config/hypr/CONTEXT.md" ]]; then
  echo ""
  echo "=== CONTEXT.md ==="
  cat "/home/yyax/dotfiles/hypr/.config/hypr/CONTEXT.md"
  echo "=== end CONTEXT.md ==="
fi
# Inject AUDIT_CONTEXT.md if recent (< 7 days) — tells Claude what to fix
AUDIT_CTX="/home/yyax/dotfiles/hypr/.config/hypr/.dual-graph/AUDIT_CONTEXT.md"
if [[ -f "$AUDIT_CTX" ]]; then
...
STORE="/home/yyax/dotfiles/hypr/.config/hypr/.dual-graph/context-store.json"
